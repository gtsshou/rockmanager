...
function applyFilters() {
    ...
    filteredData = rockData.filter(rock => {
        ...
        const textMatch = !searchText || 
            (rock.rock_type && rock.rock_type.toLowerCase().includes(searchText)) ||
            (rock.location && rock.location.toLowerCase().includes(searchText)) ||
            (rock.sample_id && rock.sample_id.toLowerCase().includes(searchText));
...
function showDetail(rockId) {
    ...
    modalTitle.textContent = `${rock.sample_id || rock.id} - ${rock.rock_type || '未知岩性'}`;
            ...
            <div class="col-md-6">
                <div class="text-center mb-3">
                    <img src="rockmanager/rocks/${rock.image}" 
                         ...
                         onerror="this.src='rockmanager/rocks/default.jpg'">
                </div>
...
function addToCompare(rockId) {
    ...
    showAlert(`已添加 ${rock.sample_id || rock.id} 到对比列表`, 'success');
}
        ...
        if (rock) {
            html += `
                <div class="compare-item">
                    <span class="small">${rock.sample_id || rock.id}</span>
